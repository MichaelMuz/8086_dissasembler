...
// Table 4-12 page 164
pub const instruction_encodings = [_]schema.InstructionSchema{
    // Data transfer
...
test "all instruction encodings parse" {
    for (instruction_encodings) |encoding| {
        _ = try parser.parser.instructionSchema(encoding.name, encoding.pattern);
    }
}

test "check that things match" {
    const mov0 = parser.parser.instructionSchema("mov0", "100010 d w") catch unreachable;
